...
<script type="text/ecmascript-6">
    ...
    methods: {
      ...
      smallClick (index, midx) {
        ...
        if (midx === 0) {
          this.$router.push({path: '/user2/knowledge/productXmind', query: query})
        } else if (midx === 1) {
          this.$router.push({path: '/user2/knowledge/knowledgeXmind', query: query})
        }
      },
      gotoPath (index, item) {
        this.smallIndex = -1
      ...
      getRoute () {
        ...
        if (this.$route.path === '/user/charles') {
          this.activeIndex = -1
          this.charlesShow = true
        } else if (this.$route.path === '/user2/corpusMenu/corpus') {
          this.activeIndex = 0
          this.charlesShow = false
        } else if (this.$route.path === '/user2/corpusMenu/addCorpus') {
          this.activeIndex = 0
          this.charlesShow = false
        } else if (this.$route.path === '/user2/corpusMenu/fileManagement') {
          this.activeIndex = 0
          this.charlesShow = false
        } else if (this.$route.path === '/user2/knowledge/productXmind' && Number(this.$route.query.index) === 0) {
          this.smallIndex = 0
          this.charlesShow = false
          this.menuArr[1].childFlag = true
        } else if (this.$route.path === '/user2/knowledge/knowledgeXmind' && Number(this.$route.query.index) === 1) {
          this.smallIndex = 1
          this.charlesShow = false
          this.menuArr[1].childFlag = true
        } else if (this.$route.path === '/user2/knowledge/productXmind') {
          this.activeIndex = 1
          this.smallIndex = 0
          this.charlesShow = false
          this.menuArr[1].childFlag = true
        } else if (this.$route.path === '/user2/knowledge/ai') {
          this.activeIndex = -1
          this.charlesShow = false
          this.smallIndex = 1
          this.menuArr[1].childFlag = true
        } else if (this.$route.path.indexOf('/knowledge/table') > -1) {
          this.activeIndex = 1
          this.charlesShow = false
          this.menuArr[1].childFlag = true
        } else if (this.$route.path === '/user2/setting') {
          this.activeIndex = 2
          this.charlesShow = false
        } else if (this.$route.path === '/user2/userinfo') {
          this.activeIndex = -1
          this.charlesShow = false
          this.smallIndex = -1
        }
