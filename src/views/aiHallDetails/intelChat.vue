      ...
      <div class="chat-w">
        <div class="chat-room clearfix" ref="chatcontainer">
          <div class="clearfix" v-for="(item, index) in chatContent" :key="index + 11111" :class="{'mySelf': item.isMy === true,  'leilei': item.isMy === false}">
            ...
            <!--          多个回答 -->
            <span v-show="item.isMy === false" v-for="(child, indexm) in item.msg" :key="indexm + 999999" class="chatbo-a">{{child}}
            ...
            <span class="chatbo-a user-a" v-show="item.isMy === true">{{item.msg}}
              <div class="evaluate-w" v-if="item.isMy === false">
                <div class="evaluate-list" :class="{'good': item.good === true}" @click="goodFn(item, index)">
                  <img src="../../assets/img/user/good_light.png" alt="" v-show="item.good === true">
                  <img src="../../assets/img/user/good_normal.png" alt=""  v-show="item.good === false">
...
<!--          多个回答 显示多个头像 -->
        ...
        <div class="bottom-ctrl" @keyup.enter="sendMsg">
          <div class="left">
            <div class="line"></div>
            <input type="text" class="sendContent" placeholder="输入聊天内容..." v-model.trim="sendContent" @input="changeInput">
          </div>
    ...
    methods: {
      ...
      showFn (item, index) {
        ...
        this.isFShow = true
		  ...
		  // 点赞
      goodFn (item, index) {
        console.log(this.chatContent, index)
