        ...
        <ul class="menu-uls">
          <li v-for="(item, index) in menuArr" :key="index" class="menu-list"
            ...
            <div class="img-editor" @click.stop="gotoEditor(item)">
              <img src="../assets/img/user/editor.png" alt="" v-if="currentIndex === index">
              <img src="../assets/img/user/editor_normal.png" alt="" v-else>
            </div>
          </li>
        </ul>
      </div>
          ...
          <div class="chart-room-content">
            ...
            <div class="m-room clearfix" ref="chatcontainer">
              <div v-for="(item, index) in chatContent" :key="index" :class="{'mySelf': item.isMy === true,  'leilei': item.isMy === false}">
                ...
                <!--          多个回答 -->
                <span>
                  <span v-show="typeof item.msg === 'object'" v-for="(child, aindex) in item.msg" :key="aindex + 66666" class="chatbo-a">{{child}}
                    <div class="evaluate-w" v-if="item.isMy === false">
                      <div class="evaluate-list" :class="{'good': item.good === true}" @click="goodFn(item, index)">
                        <img :src="require('../assets/img/user/good_light.png')" alt="" v-show="item.good === true">
                        <img :src="require('../assets/img/user/good_normal.png')" alt=""  v-show="item.good === false">
                        <span>回答不错</span>
                      </div>
                    </div>
                  </span>
                  <span class="chatbo-a user-a" v-show="typeof item.msg === 'string'">{{item.msg}}
                    <div class="evaluate-w" v-if="item.isMy === false">
                      <div class="evaluate-list" :class="{'good': item.good === true}" @click="goodFn(item, index)">
                        <img :src="require('../assets/img/user/good_light.png')" alt="" v-show="item.good === true">
                        <img :src="require('../assets/img/user/good_normal.png')" alt=""  v-show="item.good === false">
...
<!--                反馈-->
                <div v-if="item.isMy === false && !item.rule"  title="反馈">
                  <div class="editor-w"  @click="showFn(item, index)" v-for="(a, n) in item.msg" :key="n - 111111" :class="{'editor-two': n === 1}"><img src="../assets/img/user/editor.png" alt=""></div>
                </div>
                ...
                <div v-if="item.openC === true">
                  <div  class="t-f" @click="hideOrOpen(item)">{{openCText}}</div>
                  <div v-for="(a, b) in cA" :key="b" v-show="a !== ''  && item.moreD === true" class="list-t">
                    <div class="t-label">答句{{b + 1}}</div>
                    <div class="t-answer">{{a}}</div>
                  </div>
                </div>
              </div>
            </div>
    ...
    <feedback-dialog v-if="isFShow" @closeDialog="closeDialog" :chatbotIdF="chatbotIdF" :questionF="questionF" :answerF="answerF" :cuIndex="cuIndex"></feedback-dialog>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  import { Base64 } from 'js-base64';
    ...
    methods: {
	    hideOrOpen (item) {
		    item.openC = true
		    item.moreD = !item.moreD
        if (item.moreD === true) {
	        this.openCText = '收起更多修改答句'
        } else {
	        this.openCText = '展开更多修改答句'
        }
      },
      // 关闭弹窗
      closeDialog (q, a, q2, a2, index) {
        if (q) {
	        let mQ = []
	        q.forEach(item => {
		        mQ.push(item.content)
	        })
	        let mA = []
	        a.forEach(item => {
		        mA.push(item.content)
	        })
          this.openC = true
	        this.chatContent[this.chatContent.length - 2] = {
		        isMy: true,
		        msg: mQ,
            openC: false,
		        status: 3,
	        ...
	        this.chatContent[this.chatContent.length - 1] = {
		        isMy: false,
		        msg: mA,
		        openC: false,
		        moreD: false,
        ...
        if(index) {
          this.cuIndex = index
          this.chatContent[index].openC = true
          this.isFShow = false
        } else {
          this.isFShow = false
        }
      ...
      showFn (item, index) {
        this.chatbotIdF = this.chatbotsOne
        this.questionF = [this.chatContent[index - 1].msg]
        // let answerF = []
        // if (typeof this.chatContent[index].msg === 'String') {
        //   answerF.push(this.chatContent[index].msg)
        // } else {
        //   answerF = this.chatContent[index].msg
        // }
        this.answerF = this.chatContent[index].msg
        this.cuIndex = index
        this.isFShow = true
      },
      goodFn (item, index) {
        let question = this.chatContent[index - 1].msg
        item.good = true
        if (item.good === true) {
